import { Component } from '@angular/core';
import { CartService } from '../cart.service';
import { Router } from '@angular/router';
@Component({
  selector: 'app-detergents',
  templateUrl: './detergents.component.html',
  styleUrls: ['./detergents.component.css']
})
export class DetergentsComponent {
  constructor(public cartService: CartService, private router: Router) {
    this.cartService.cartItems$.subscribe(cartItems => {
      console.log('Cart Items:', cartItems);
      console.log('Total Price:', this.cartService.calculateTotal());
    });
  }

  buy(detergentName: string, price: number) {
    // Add the selected fruit to the cart
    const selectedItem = { name: detergentName, price: price };
    this.cartService.addToCart(selectedItem);
    this.router.navigate(['cart']);

  }

}
